...
module.exports = {
    start: function (params, callback) {
        ...
        var server = http.createServer(function (req, res) {
            ...
            var serverCurrent = servers[req.headers.host.split(":")[0]];
            if (serverCurrent) {
                realPath = path.join(serverCurrent.basePath, path.normalize(pathname.replace(/\.\./g, "")));
            } else {
                console.error('The server of "' + req.headers.host.split(":")[0] + '" is not defined, please check the config.js!');
                return false;
            }
            //匹配忽略列表，若匹配直接抛给回调函数
            if (serverCurrent.ignoreRegExp && req.url.match(serverCurrent.ignoreRegExp)) {
